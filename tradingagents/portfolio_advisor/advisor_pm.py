...
def run_pm_cycle(
    ...
    *,
    ...
    # Detect deposits / withdrawals by diffing cash + total vs last snapshot.
    _cur_cash = _parse_available_balance(portfolio_text)
    _cur_total = None
    try:
        _ubvs = [float(r['unitsBaseValueDollars']) for r in portfolio_rows
                 if r.get('unitsBaseValueDollars') is not None]
        _cur_total = (sum(_ubvs) + (_cur_cash or 0.0)) if _ubvs or _cur_cash is not None else None
    except Exception:
        _cur_total = None
    _sleeve_actuals = {'core': 0.0, 'catalyst': 0.0, 'cash': float(_cur_cash or 0.0)}
    try:
        _plans_now = pos_plans.load_position_plans(cfg)
        for _r in portfolio_rows:
            _ubv = _float_or_none(_r.get('unitsBaseValueDollars'))
            if _ubv is None:
                continue
            _tk2 = _normalize_ticker(str(_r.get('symbolFull') or ''))
            _strat = (_plans_now.get(_tk2).strategy if _plans_now.get(_tk2) else 'core')
            if _strat == 'catalyst':
                _sleeve_actuals['catalyst'] += _ubv
            else:
                _sleeve_actuals['core'] += _ubv
    except Exception:
        logger.debug('sleeve_actuals build failed', exc_info=True)
    cash_change_blk = _pmws.load_cash_change_block(
        cfg, current_cash=_cur_cash, current_total=_cur_total,
        sleeve_actuals=_sleeve_actuals,
    )
    strategy_block = (
    ...
    # Macro evidence — honest pattern display with n-counts.
    try:
        from tradingagents.portfolio_advisor.macro_learning import build_evidence_block
        evidence_blk = build_evidence_block(cfg)
    except Exception as e:
...
"""
    ...
    try:
        ...
        pass
    try:
        _pmws.update_cash_snapshot(cfg, current_cash=_cur_cash, current_total=_cur_total)
    except Exception:
        logger.debug('update_cash_snapshot failed', exc_info=True)
...
def run_action_check(cfg: Dict[str, Any]) -> "AdvisorPMCycleResult":
    ...
    # Evaluate consensus guardrail kill switch before PM cycle
    if cfg.get("CONSENSUS_GUARDRAILS_LIVE", False):
        try:
            _evaluate_kill_switch(cfg)
        except Exception:
            logger.debug("action_check: kill switch evaluation failed", exc_info=True)
...
# --- Consensus Guardrails: Kill Switch & Mode State Machine (Phase E) ---

def _evaluate_kill_switch(cfg: Dict[str, Any]) -> None:
    """Evaluate kill switch conditions and transition system mode.

    Entry: any condition true for 5 consecutive PM cycles.
    Exit: all conditions false for 10 consecutive PM cycles.
    Minimum dwell: 14 calendar days in defensive mode.
    """
    from datetime import date as _date, timedelta as _td

    try:
        from tradingagents.dataflows.llm_consensus import load_llm_consensus_snapshot
        from tradingagents.portfolio_advisor.state import load_system_mode, save_system_mode
        ...
        if consensus:
            # Condition 1: DeepSeek alignment > 80%
            alignment = consensus.get("deepseek_alignment", {})
            overlap = float(alignment.get("overlap_with_top_20", 0) or 0)
            if overlap > 0.80:
                conditions_met = True
                logger.info("kill switch: DeepSeek alignment %.1f%% > 80%%", overlap * 100)

            # Condition 2: >60% portfolio in consensus top 20
            try:
                from tradingagents.portfolio_advisor.etoro_scan import fetch_portfolio_rows
                _, _, _, rows = fetch_portfolio_rows()
                top_20 = {t["ticker"] for t in consensus.get("top_20", [])}
                total_val = sum(abs(float(r.get("invested_usd", 0) or r.get("initialAmountInDollars", 0))) for r in rows)
                consensus_val = sum(abs(float(r.get("invested_usd", 0) or r.get("initialAmountInDollars", 0)))
                                   for r in rows
                                   if str(r.get("symbolFull", "") or r.get("ticker", "")).strip().upper() in top_20)
                if total_val > 0 and consensus_val / total_val > 0.60:
                    conditions_met = True
                    logger.info("kill switch: %.0f%% portfolio in consensus top 20", consensus_val / total_val * 100)
            except Exception:
                pass

        # Track condition history
        history_path = Path.home() / ".tradingagents" / "state" / "kill_switch_history.json"
        history: list = []
        ...
        history.append({"date": today, "conditions_met": conditions_met})
        ...
        consecutive_clear = 0
        for entry in reversed(history):
            if not entry["conditions_met"]:
                consecutive_clear += 1
            else:
                break

        # Mode transitions
        if mode == "normal" and consecutive_met >= 5:
            save_system_mode("consensus_defensive", today)
            logger.warning("KILL SWITCH: entering consensus_defensive mode after %d consecutive condition days", consecutive_met)
            try:
                from tradingagents.portfolio_advisor.messaging import send_telegram_message
                send_telegram_message(cfg, "SYSTEM MODE CHANGE",
                    f"Entered CONSENSUS_DEFENSIVE mode. {consecutive_met} consecutive days with kill switch conditions met. "
                    f"New consensus entries blocked. Minimum dwell 14 days.")
            except Exception:
                pass

        elif mode == "consensus_defensive" and consecutive_clear >= 10:
            # Check minimum dwell
            state_path = Path.home() / ".tradingagents" / "state" / "system_mode.json"
            entered_at_str = ""
            ...
            if entered_at_str:
                try:
                    entered_date = _date.fromisoformat(entered_at_str[:10])
                    if (_date.today() - entered_date).days < 14:
                        logger.info("kill switch: dwell not met (%d days < 14)", (_date.today() - entered_date).days)
                        return
                except (ValueError, TypeError):
                    pass

            save_system_mode("normal", today)
            logger.warning("KILL SWITCH: exiting consensus_defensive mode after %d consecutive clear days", consecutive_clear)
            try:
                from tradingagents.portfolio_advisor.messaging import send_telegram_message
                send_telegram_message(cfg, "SYSTEM MODE CHANGE",
                    f"Returned to NORMAL mode. {consecutive_clear} consecutive days with conditions cleared.")
            except Exception:
                pass

    except Exception:
        logger.debug("kill switch evaluation failed", exc_info=True)
